  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      return NextResponse.json(
        { ok: false, error: "Falta configurar OPENAI_API_KEY en el servidor." },
        { status: 500 }
      );
    }
    ...
    if (!diagnosticImpression) {
      return NextResponse.json(
        { ok: false, error: "Debes escribir una impresión final." },
        { status: 400 }
      );
    }
    ...
    const model = process.env.OPENAI_MODEL_EVALUATION || "gpt-5-mini";
    ...
    const evaluation =
      response.output_text?.trim() || "No se pudo generar la evaluación.";
